...
public:
  constexpr vector() : _size(N) { std::fill(_data.begin(), _data.end(), T()); }
  ...
  template <size_t M> constexpr vector(const vector<T, M> &content) {
    resize(content.size());
    std::copy(content.begin(), content.end(), _data.begin());
  }
...
public:
  template <class... Args> void emplace(Args &&...args) {
    const auto pos = std::find_if(this->begin(), this->end(),
  ...
  template <typename F> void erase_if_key(F &&f) {
...
template <size_t N> using owned_bytes = vector<uint8_t, N>;
...
struct KeyAndSalt {
  static KeyAndSalt from_base_key(CipherSuite suite, KeyID key_id,
...
public:
  ...
  output_bytes protect(const Header &header, output_bytes ciphertext,
                       input_bytes plaintext, input_bytes metadata);
  output_bytes unprotect(const Header &header, output_bytes ciphertext,
                         input_bytes plaintext, input_bytes metadata);
...
public:
  ...
  output_bytes protect(KeyID key_id, output_bytes ciphertext,
                       input_bytes plaintext, input_bytes metadata);
...
public:
  ...
  output_bytes protect(EpochID epoch_id, SenderID sender_id,
                       output_bytes ciphertext, input_bytes plaintext,
                       input_bytes metadata);
  output_bytes protect(EpochID epoch_id, SenderID sender_id,
                       ContextID context_id, output_bytes ciphertext,
                       input_bytes plaintext, input_bytes metadata);
...
private:
  struct EpochKeys {
    ...
    EpochKeys(EpochID full_epoch_in, input_bytes sframe_epoch_secret_in,
              size_t epoch_bits, size_t sender_bits_in);
  ...
  KeyID form_key_id(EpochID epoch_id, SenderID sender_id,
