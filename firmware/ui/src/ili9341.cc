...
// NOTE, we never deselect the ILI9341 because its the only
...
void ILI9341::Init()
{
    ...
    // Set the orientation of the ILI9341
...
void ILI9341::Draw(uint32_t timeout)
{
    UNUSED(timeout);
    if (restart_update)
    {
    ...
    for (uint16_t j = 0; j < Num_Memories && memories_read < num_memories_used; ++j)
    {
        ...
        default:
        {
            ...
            Error("ILI9341 Draw", "Should be impossible to get here!");
            break;
        }
        }
    }
...
void ILI9341::SetWriteablePixels(const int16_t x1,
                                 const int16_t x2,
                                 const int16_t y1,
                                 const int16_t y2)
{
...
void ILI9341::SetOrientation(const ILI9341::Orientation orientation)
{
    // TODO fix
...
void ILI9341::FillRectangle(uint16_t x1, uint16_t x2, uint16_t y1, uint16_t y2, const Colour colour)
{
...
void ILI9341::FillScreen(const Colour colour)
{
    FillRectangle(0, view_width, 0, view_height, colour);
}

void ILI9341::DrawRectangle(uint16_t x1,
                            uint16_t x2,
                            uint16_t y1,
                            uint16_t y2,
...
void ILI9341::DrawString(uint16_t x,
                         uint16_t y,
                         const char* str,
...
void ILI9341::DefineScrollArea(const uint16_t tfa_idx,
...
void ILI9341::ScrollScreen(const uint16_t scroll_idx, bool up)
{
...
void ILI9341::UpdateTitle(const char* title, const uint32_t len)
{
...
void ILI9341::AppendText(const char* text, const uint32_t len)
{
    // Work our way down
...
void ILI9341::CommitText()
{
    ...
    if (texts_in_use >= Max_Texts)
    {
        ...
        // Draw a rectangle to remove the text from the ILI9341.
...
void ILI9341::CommitText(const char* text, const uint32_t len)
{
    AppendText(text, len);
    CommitText();
}

void ILI9341::AppendUserText(const char* text, const uint32_t len)
{
...
void ILI9341::AppendUserText(const char ch)
{
    if (usr_buffer_idx >= Max_Characters)
    {
        return;
...
void ILI9341::BackspaceUserText()
{
    if (usr_buffer_idx == 0)
    {
        return;
...
const char* ILI9341::UserText() const noexcept
{
    return usr_buffer;
}

uint16_t ILI9341::UserTextLength() const noexcept
{
...
inline void ILI9341::WaitForSPIComplete()
{
...
inline void ILI9341::SetPinToCommand()
{
    HAL_GPIO_WritePin(dc_port, dc_pin, GPIO_PIN_RESET);
}

inline void ILI9341::SetPinToData()
{
...
void ILI9341::WriteCommand(uint8_t cmd, uint8_t* data, const uint32_t sz)
{
    WriteCommand(cmd);
    WriteData(data, sz);
}
...
void ILI9341::WriteData(uint8_t* data, const uint32_t data_size)
{
    WaitForSPIComplete();
    HAL_SPI_Transmit_DMA(spi, data, data_size);
}

void ILI9341::WriteDataWithSet(uint8_t data)
{
    WaitForSPIComplete();
    SetPinToData();
...
void ILI9341::WriteDataWithSet(uint8_t* data, const uint32_t data_size)
{
    WaitForSPIComplete();
    SetPinToData();
    HAL_SPI_Transmit_DMA(spi, data, data_size);
...
ILI9341::DrawMemory& ILI9341::AllocateMemory(const uint16_t x1,
                                             const uint16_t x2,
                                             const uint16_t y1,
                                             const uint16_t y2,
                                             const Colour colour,
                                             MemoryCallback callback)
{
    ...
    {
        Error("ILI9341 Draw", "Num memories overflow");
    }
    ...
    ILI9341::DrawMemory& memory = memories[memory_write_idx++];
...
void ILI9341::NormalMode()
{
    // Send a dma command
}

bool ILI9341::FillRectangleProcedure(DrawMemory& memory,
...
void ILI9341::DrawRectangleProcedure(const int16_t x1,
                                     const int16_t x2,
                                     const int16_t y1,
                                     const int16_t y2,
                                     const uint16_t thickness,
...
bool ILI9341::DrawRectangleProcedure(DrawMemory& memory,
                                     uint8_t matrix[HEIGHT][Half_Width_Pixel_Size],
                                     const int16_t y1,
                                     const int16_t y2)
{
...
bool ILI9341::DrawCharacterProcedure(DrawMemory& memory,
                                     uint8_t matrix[HEIGHT][Half_Width_Pixel_Size],
                                     const int16_t y1,
                                     const int16_t y2)
{
...
bool ILI9341::DrawStringProcedure(DrawMemory& memory,
                                  uint8_t matrix[HEIGHT][Half_Width_Pixel_Size],
                                  const int16_t y1,
                                  const int16_t y2)
{
...
void ILI9341::HandleBounds(uint16_t& x1, uint16_t& x2, uint16_t& y1, uint16_t& y2)
{
...
inline uint8_t* ILI9341::GetCharAddr(uint8_t* font_data,
                                     const uint8_t ch,
...
inline void
ILI9341::PushMemoryParameter(DrawMemory& memory, const uint32_t val, const int16_t num_bytes)
{
...
inline void ILI9341::FillMatrixAtIdx(uint8_t matrix[HEIGHT][Half_Width_Pixel_Size],
                                     const uint16_t i,
                                     const uint16_t j,
...
inline ILI9341::YBound ILI9341::GetYBounds(const uint16_t y1,
                                           const uint16_t y2,
