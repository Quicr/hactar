...
// void Screen::FillRectangle(uint16_t x1, uint16_t x2, uint16_t y1, uint16_t y2, const Colour
// colour)
// {
...
//     const uint16_t len =
//         x + length * static_cast<uint16_t>(font.width) > WIDTH ? (WIDTH - x) / font.width :
//         length;
