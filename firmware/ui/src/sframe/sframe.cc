...
static auto from_ascii(const char* str, size_t len)
{
...
static owned_bytes<32> sframe_key_label(CipherSuite suite, KeyID key_id)
{
...
static owned_bytes<33> sframe_salt_label(CipherSuite suite, KeyID key_id)
{
...
void Context::add_key(KeyID key_id, KeyUsage usage, input_bytes base_key)
{
    keys.emplace(key_id, KeyRecord::from_base_key(suite, key_id, usage, base_key));
}

static owned_bytes<KeyRecord::max_salt_size> form_nonce(Counter ctr, input_bytes salt)
{
...
static owned_bytes<max_aad_size> form_aad(const Header& header, input_bytes metadata)
{
...
output_bytes
Context::protect(KeyID key_id, output_bytes ciphertext, input_bytes plaintext, input_bytes metadata)
{
    ...
    auto final_ciphertext = Context::protect_inner(header, inner_ciphertext, plaintext, metadata);
    return ciphertext.first(header_data.size() + final_ciphertext.size());
}

output_bytes
Context::unprotect(output_bytes plaintext, input_bytes ciphertext, input_bytes metadata)
{
...
output_bytes Context::protect_inner(const Header& header,
...
output_bytes Context::unprotect_inner(const Header& header,
...
void MLSContext::add_epoch(EpochID epoch_id, input_bytes sframe_epoch_secret)
{
    add_epoch(epoch_id, sframe_epoch_secret, 0);
}

void MLSContext::add_epoch(EpochID epoch_id, input_bytes sframe_epoch_secret, size_t sender_bits)
{
...
void MLSContext::purge_before(EpochID keeper)
{
...
output_bytes MLSContext::protect(EpochID epoch_id,
                                 SenderID sender_id,
                                 output_bytes ciphertext,
                                 input_bytes plaintext,
                                 input_bytes metadata)
...
output_bytes
MLSContext::unprotect(output_bytes plaintext, input_bytes ciphertext, input_bytes metadata)
{
    const auto header = Header::parse(ciphertext);
    const auto inner_ciphertext = ciphertext.subspan(header.size());
...
MLSContext::EpochKeys::base_key(CipherSuite ciphersuite, SenderID sender_id) const
{
    const auto hash_size = cipher_digest_size(ciphersuite);
    auto enc_sender_id = owned_bytes<8>();
    encode_uint(sender_id, enc_sender_id);

    return hkdf_expand(ciphersuite, sframe_epoch_secret, enc_sender_id, hash_size);
}

void MLSContext::purge_epoch(EpochID epoch_id)
{
    const auto drop_bits = epoch_id & epoch_bits;

    keys.erase_if_key([&](const auto& epoch) { return (epoch & epoch_bits) == drop_bits; });
}

KeyID MLSContext::form_key_id(EpochID epoch_id, SenderID sender_id, ContextID context_id) const
{
    auto epoch_index = epoch_id & epoch_mask;
    auto& epoch = epoch_cache[epoch_index];
    if (!epoch)
    {
        throw invalid_parameter_error("Unknown epoch. epoch_index: " + std::to_string(epoch_index)
...
void MLSContext::ensure_key(KeyID key_id, KeyUsage usage)
{
    ...
    if (!epoch)
    {
        throw invalid_parameter_error("Unknown epoch: " + std::to_string(epoch_index));
    }
