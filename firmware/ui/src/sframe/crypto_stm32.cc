...
hkdf_extract(CipherSuite suite, input_bytes salt, input_bytes ikm)
{
    ...
    VERIFY_CMOX_CALL(cmox_mac_compute(
        cmox_hmac_algo(suite),
        ikm.data(),
        ikm.size(),
        salt.data(),
        salt.size(),
...
hkdf_expand(CipherSuite suite, input_bytes prk, input_bytes info, size_t size)
{
    ...
    for (uint8_t i = 1; i <= N; i++)
    {
        ...
        if (i > 1)
        {
            VERIFY_CMOX_CALL(cmox_mac_append(mac_ctx, computed_hash.data(), digest_size), CMOX_MAC_SUCCESS);
        }
        ...
        VERIFY_CMOX_CALL(cmox_mac_generateTag(mac_ctx, computed_hash.data(), nullptr), CMOX_MAC_SUCCESS);
          ...
          input_bytes pt)
{
    ...
    VERIFY_CMOX_CALL(cmox_aead_encrypt(
          ...
          input_bytes ct)
{
  ...
  VERIFY_CMOX_CALL(cmox_aead_decrypt(
