...
crypto_error::crypto_error() :
    std::runtime_error(cmox_error.has_value()
                           ? "CMOX crypto error (error=" + std::to_string(cmox_error.value()) + ")"
                           : "unknown CMOX crypto error")
{
}

cmox_mac_algo_t cmox_hmac_algo(CipherSuite suite)
{
    switch (suite)
    {
...
cmox_hmac_impl_t cmox_hmac_impl(CipherSuite suite)
{
    switch (suite)
    {
    case CipherSuite::AES_128_CTR_HMAC_SHA256_80:
...
std::size_t cmox_hmac_size(CipherSuite suite)
{
    switch (suite)
    {
    case CipherSuite::AES_128_CTR_HMAC_SHA256_80:
...
hkdf_extract(CipherSuite suite, input_bytes salt, input_bytes ikm)
{
    ...
    VERIFY_CMOX_CALL(cmox_mac_compute(cmox_hmac_algo(suite), ikm.data(), ikm.size(), salt.data(),
                                      salt.size(), nullptr, 0, out.data(), out.size(),
                                      &computed_size),
                     CMOX_MAC_SUCCESS);
...
hkdf_expand(CipherSuite suite, input_bytes prk, input_bytes info, size_t size)
{
    ...
    for (uint8_t i = 1; i <= N; i++)
    {
        ...
        if (i == N)
        {
            VERIFY_CMOX_CALL(cmox_mac_setTagLen(mac_ctx, size - index), CMOX_MAC_SUCCESS);
        }

        VERIFY_CMOX_CALL(cmox_mac_setKey(mac_ctx, prk.data(), prk.size()), CMOX_MAC_SUCCESS);

        if (i > 1)
        {
            VERIFY_CMOX_CALL(cmox_mac_append(mac_ctx, computed_hash.data(), digest_size),
                             CMOX_MAC_SUCCESS);
        }

        VERIFY_CMOX_CALL(cmox_mac_append(mac_ctx, info.data(), info.size()), CMOX_MAC_SUCCESS);
        VERIFY_CMOX_CALL(cmox_mac_append(mac_ctx, &i, 1), CMOX_MAC_SUCCESS);
        VERIFY_CMOX_CALL(cmox_mac_generateTag(mac_ctx, computed_hash.data(), nullptr),
                         CMOX_MAC_SUCCESS);
...
static output_bytes seal_aead(CipherSuite suite,
                              input_bytes key,
                              input_bytes nonce,
                              output_bytes ct,
                              input_bytes aad,
                              input_bytes pt)
{
    ...
    VERIFY_CMOX_CALL(cmox_aead_encrypt(CMOX_AESFAST_GCMFAST_ENC_ALGO, pt.data(), pt.size(),
                                       tag_size, key.data(), key.size(), nonce.data(), nonce.size(),
                                       aad.data(), aad.size(), ct.data(), &computed_size),
                     CMOX_CIPHER_SUCCESS);
...
static output_bytes open_aead(CipherSuite suite,
                              input_bytes key,
                              input_bytes nonce,
                              output_bytes pt,
                              input_bytes aad,
                              input_bytes ct)
{
    ...
    VERIFY_CMOX_CALL(cmox_aead_decrypt(CMOX_AESFAST_GCMFAST_DEC_ALGO, ct.data(), ct.size(),
                                       tag_size, key.data(), key.size(), nonce.data(), nonce.size(),
                                       aad.data(), aad.size(), pt.data(), &computed_size),
