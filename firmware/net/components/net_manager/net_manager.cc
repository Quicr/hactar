...
void NetManager::HandleSerial(void* param)
{
    ...
    while (true)
    {
        ...
        while (self->audio_buffer.Unread() > 15 || self->audio_buffered)
        {
            ...
            self->ui_layer.EnqueuePacket(std::move(self->audio_buffer.Read()));
            self->ui_layer.Update(xTaskGetTickCount() / portTICK_PERIOD_MS);
        }
...
void NetManager::HandleSerialCommands()
{
    ...
    while (wifi_packets->Unread() > 0)
    {
        ...
        std::unique_ptr<SerialPacket> packet = std::move(wifi_packets->Read());
        // Get the type
    ...
    while (room_packets->Unread() > 0)
    {
        ...
        std::unique_ptr<SerialPacket> packet = std::move(room_packets->Read());
