...
//         {
//             memcpy(stream->tx->buff + stream->tx->write, stream->rx->buff + stream->rx->idx,
//             bytes); stream->tx->unsent += bytes;
//         }
...
//         memcpy(stream->tx->buff + stream->tx->write, stream->rx->buff + stream->rx->idx,
//         num_bytes); stream->tx->unsent += num_bytes;
//     }
...
// void TxISR(transmit_t* tx, enum State* state)
// {
...
// void Transmit(transmit_t* tx, enum State* state)
// {
//     if (tx->unsent == 0 || quiet)
//     {
//         tx->free = 1;
//         return;
//     }
...
//         {
//             HAL_UART_Transmit(stream->rx->uart,
//                               (const uint8_t*)"ERROR! configration is too short\n", 33,
//                               HAL_MAX_DELAY);
//             goto cleanup;
//         }
